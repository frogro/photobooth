#!/bin/bash
# Universal Photobooth installer v8 - Ubuntu 26/PHP auto + Node 20 + safer permissions
...
BRANCH="sumup-coin"
REMOTE_BRANCH_API="https://api.github.com/repos/frogro/photobooth/branches/${BRANCH}"
REMOTE_BRANCH_SHA=""
...
# SumUp / Webhook
SUMUP_WEBHOOK_PACKAGES=(
    "python3-flask"
    "python3-requests"
)

SUMUP_GUNICORN_PACKAGES=(
    "gunicorn"
)
...
function info() {
    ...
    if [ "$SILENT" = true ]; then
        echo "$title: $message"
    elif [ "$QUIET_INFOBOXES" = true ]; then
        :
    else
...
function show_help() {
    ...
    echo "  --php=<version>             Set the PHP version for the setup (e.g., --php=8.3).
                              Default: auto (use distribution default PHP)."
    echo "  --silent                    Run the Photobooth Setup Wizard in silent mode"
    echo "                              for automated installation or updates."
    echo "  --username=\"<username>\"     Optional in silent mode. If omitted, the script tries to auto-detect"
    echo "                              the invoking sudo user or the single user under /home."
...
function install_package() {
    ...
    # Ubuntu 26.04 may ship Node.js as a newer distro package (e.g. Node 22).
    # If node and npm are already usable, do not try to reinstall/downgrade nodejs.
    # This also avoids apt conflicts with corepack/nsolid packages.
    if [[ "$package" == "nodejs" ]]; then
        if command -v node >/dev/null 2>&1 && command -v npm >/dev/null 2>&1; then
            info "Package installation" "nodejs is already available: $(node -v 2>/dev/null), npm $(npm -v 2>/dev/null)."
            return 0
        fi
    fi
...
function install_sumup_webhook_dependencies() {
    ...
    info "SumUp Setup" "SumUp webhook dependencies installed successfully."
    return 0
}

function install_sumup_gunicorn() {
    if [ "$SKIP_GUNICORN" = true ]; then
        info "Gunicorn Setup" "Skipping Gunicorn installation."
        return 0
    fi
    ...
    info "Gunicorn Setup" "Gunicorn installed successfully."
    return 0
}

function install_ngrok() {
    if [ "$SKIP_NGROK" = true ]; then
        info "ngrok Setup" "Skipping ngrok installation."
        return 0
    fi

    if command -v ngrok >/dev/null 2>&1; then
        info "ngrok Setup" "ngrok is already installed."
        return 0
    fi

    local ngrok_repo_codename="$OS_CODENAME"
    case "$ngrok_repo_codename" in
        bullseye|bookworm|trixie) ;;
        *) ngrok_repo_codename="bookworm" ;;
    esac

    info "ngrok Setup" "Installing ngrok from official repository for $ngrok_repo_codename..."
    ...
    if ! echo "deb https://ngrok-agent.s3.amazonaws.com ${ngrok_repo_codename} main" | tee /etc/apt/sources.list.d/ngrok.list >/dev/null; then
    ...
    info "ngrok Setup" "ngrok installed successfully."
    info "ngrok Setup" "Please run 'ngrok config add-authtoken <YOUR_TOKEN>' once if not done already."
    return 0
}

function create_sumup_token_file() {
    if ! check_installfolderpath; then
        return 1
    fi

    local token_file="$INSTALLFOLDERPATH/config/sumup_token.txt"

    if [[ ! -f "$token_file" ]]; then
        : > "$token_file" || return 1
        info "SumUp Setup" "Created empty SumUp token file at $token_file"
    else
        info "SumUp Setup" "SumUp token file already exists at $token_file"
    fi

    chown www-data:www-data "$token_file" >/dev/null 2>&1 || warn "Failed to set ownership on $token_file"
    chmod 660 "$token_file" >/dev/null 2>&1 || warn "Failed to set permissions on $token_file"
    return 0
}
...
function create_sumup_gunicorn_service() {
    ...
    if [ "$SKIP_GUNICORN" = true ]; then
        info "Gunicorn Service" "Skipping Gunicorn service setup."
        return 0
    fi

    if [[ ! -f "$INSTALLFOLDERPATH/api/webhook_sumup.py" ]]; then
        warn "Gunicorn Service" "webhook_sumup.py not found. Skipping Gunicorn service setup."
        return 1
    fi

    cat >/etc/systemd/system/photobooth-sumup-gunicorn.service <<EOF
[Unit]
Description=Photobooth SumUp Webhook (Gunicorn)
After=network.target

[Service]
User=www-data
Group=www-data
WorkingDirectory=$INSTALLFOLDERPATH/api
ExecStart=/usr/bin/gunicorn -w 2 -b 127.0.0.1:5000 webhook_sumup:app
Restart=always
RestartSec=3
...
EOF
    ...
    if [[ "$HAS_SYSTEMD" == true ]]; then
        ...
        if ! systemctl enable photobooth-sumup-gunicorn.service >/dev/null 2>&1; then
            return 3
        fi
        info "Gunicorn Service" "photobooth-sumup-gunicorn.service installed."
    else
        mkdir -p /etc/systemd/system/multi-user.target.wants
        ln -sf /etc/systemd/system/photobooth-sumup-gunicorn.service /etc/systemd/system/multi-user.target.wants/photobooth-sumup-gunicorn.service || return 4
    fi
...
function create_ngrok_service() {
    ...
    if [[ -z "$ngrok_url" && "$SILENT" = false ]]; then
        ngrok_url=$(whiptail --title "ngrok URL" \
            --inputbox "Enter your reserved ngrok URL without https://\nExample: myurl.ngrok-free.app" \
            10 70 "" 3>&1 1>&2 2>&3)
    fi
    ...
    if [[ -z "$ngrok_url" ]]; then
        warn "ngrok Service" "No ngrok URL configured. Service file will not be created."
        return 1
    fi
...
EOF
    ...
    cat >/etc/systemd/system/photobooth-sumup-ngrok.service <<'EOF'
[Unit]
Description=Photobooth SumUp ngrok Tunnel
After=network.target photobooth-sumup-gunicorn.service
Wants=photobooth-sumup-gunicorn.service

[Service]
EnvironmentFile=/etc/default/photobooth-sumup-ngrok
ExecStart=/usr/bin/env ngrok http 5000 --url=https://${NGROK_URL}
Restart=always
RestartSec=5
...
EOF
    ...
    if [[ "$HAS_SYSTEMD" == true ]]; then
        systemctl daemon-reload >/dev/null 2>&1 || return 2
        if ! systemctl enable photobooth-sumup-ngrok.service >/dev/null 2>&1; then
            return 3
        fi
        info "ngrok Service" "photobooth-sumup-ngrok.service installed."
    else
        mkdir -p /etc/systemd/system/multi-user.target.wants
        ln -sf /etc/systemd/system/photobooth-sumup-ngrok.service /etc/systemd/system/multi-user.target.wants/photobooth-sumup-ngrok.service || return 4
    fi

    return 0
}

function manage_ngrok_gunicorn() {
    local gunicorn_service="photobooth-sumup-gunicorn.service"
    local ngrok_service="photobooth-sumup-ngrok.service"

    while true; do
        MENU_OPTIONS=(
            "1" "Install / configure Gunicorn service"
            "2" "Install / configure ngrok service"
            "3" "Start both services"
            "4" "Stop both services"
            "5" "Restart both services"
            "6" "Show both services status"
        )

        if ! CHOICE=$(whiptail --title "Ngrok / Gunicorn" \
        ...
        case $CHOICE in
            1)
                create_sumup_gunicorn_service
                case $? in
                    0) confirm "Gunicorn Service" "Gunicorn service configured successfully." ;;
                    1) confirm "Gunicorn Service" "Failed because webhook_sumup.py or installation path is missing." ;;
                    2) confirm "Gunicorn Service" "Failed to reload systemd daemon." ;;
                    3) confirm "Gunicorn Service" "Failed to enable Gunicorn service." ;;
                    4) confirm "Gunicorn Service" "Failed to create Gunicorn service symlink." ;;
                    *) confirm "Gunicorn Service" "An unknown error occurred." ;;
                esac
                ;;
            2)
                create_ngrok_service
                case $? in
                    0) confirm "ngrok Service" "ngrok service configured successfully." ;;
                    1) confirm "ngrok Service" "Failed because ngrok is missing or no URL was provided." ;;
                    2) confirm "ngrok Service" "Failed to reload systemd daemon." ;;
                    3) confirm "ngrok Service" "Failed to enable ngrok service." ;;
                    4) confirm "ngrok Service" "Failed to create ngrok service symlink." ;;
                    *) confirm "ngrok Service" "An unknown error occurred." ;;
                esac
                ;;
            3)
                if [[ "$HAS_SYSTEMD" == true ]]; then
                    systemctl start "$gunicorn_service" "$ngrok_service" >/dev/null 2>&1
                    confirm "Ngrok / Gunicorn" "Start command sent."
                else
                    confirm "Ngrok / Gunicorn" "systemd is not available on this system."
                fi
                ;;
            4)
                if [[ "$HAS_SYSTEMD" == true ]]; then
                    systemctl stop "$ngrok_service" "$gunicorn_service" >/dev/null 2>&1
                    confirm "Ngrok / Gunicorn" "Stop command sent."
                else
                    confirm "Ngrok / Gunicorn" "systemd is not available on this system."
                fi
                ;;
            5)
                if [[ "$HAS_SYSTEMD" == true ]]; then
                    systemctl restart "$gunicorn_service" "$ngrok_service" >/dev/null 2>&1
                    confirm "Ngrok / Gunicorn" "Restart command sent."
                else
                    confirm "Ngrok / Gunicorn" "systemd is not available on this system."
                fi
                ;;
            6)
                if [[ "$HAS_SYSTEMD" == true ]]; then
                    local status_text
                    status_text="Gunicorn: $(systemctl is-active "$gunicorn_service" 2>/dev/null || echo unknown)\nngrok: $(systemctl is-active "$ngrok_service" 2>/dev/null || echo unknown)"
                    confirm "Ngrok / Gunicorn" "$status_text" 12 70
                else
                    confirm "Ngrok / Gunicorn" "systemd is not available on this system."
                fi
                ;;
...
function check_username() {
    while true; do
        # Try auto-detect first. In silent sudo runs, who -m may be empty on some desktops,
        # so prefer SUDO_USER/logname before falling back to a single /home user.
        if [ -z "$USERNAME" ]; then
            if [[ -n "${SUDO_USER:-}" && "${SUDO_USER:-}" != "root" && -d "/home/${SUDO_USER}" ]]; then
                USERNAME="$SUDO_USER"
                info "Setup Wizard" "Automatically detected sudo username: $USERNAME"
            else
                local logname_user=""
                logname_user=$(logname 2>/dev/null || true)
                if [[ -n "$logname_user" && "$logname_user" != "root" && -d "/home/$logname_user" ]]; then
                    USERNAME="$logname_user"
                    info "Setup Wizard" "Automatically detected login username: $USERNAME"
                else
                    local detected_user
...
function detect_installed_php_version() {
...
function detect_default_php_candidate_version() {
    local candidate=""
    candidate=$(apt-cache policy php-cli 2>/dev/null | awk '/Candidate:/ {print $2; exit}')
    if [[ -n "$candidate" && "$candidate" != "(none)" ]]; then
        echo "$candidate" | sed -E 's/^([0-9]+:)?([0-9]+\.[0-9]+).*/\2/'
        return 0
    fi
    return 1
}

function rebuild_php_package_lists() {
    local version="${1:-$PHP_VERSION_EFFECTIVE}"

    if [[ -z "$version" || "$version" == "auto" || "$version" == "default" ]]; then
        APACHE_PACKAGES=(
            "apache2"
            "libapache2-mod-php"
        )
...
function resolve_php_version() {
    # PHP_VERSION=auto/default means: use the distribution default PHP.
    # This keeps Ubuntu 26.04+ working (currently PHP 8.5) while preserving
    # explicit old behavior via --php=8.4, --php=8.3, etc.
    if [[ -z "$PHP_VERSION" || "$PHP_VERSION" == "auto" || "$PHP_VERSION" == "default" ]]; then
        PHP_VERSION="auto"

        if PHP_VERSION_EFFECTIVE=$(detect_installed_php_version); then
            info "PHP detection" "Detected installed PHP CLI version: $PHP_VERSION_EFFECTIVE"
        elif PHP_VERSION_EFFECTIVE=$(detect_default_php_candidate_version); then
            info "PHP detection" "Detected distro default PHP candidate: $PHP_VERSION_EFFECTIVE"
        else
            PHP_VERSION_EFFECTIVE=""
            warn "Could not detect distro default PHP version. Falling back to generic PHP packages."
        fi

        rebuild_php_package_lists ""
    else
        PHP_VERSION_EFFECTIVE="$PHP_VERSION"
        rebuild_php_package_lists "$PHP_VERSION_EFFECTIVE"
        info "PHP detection" "Using requested PHP version: $PHP_VERSION_EFFECTIVE"
    fi
}
...
function prepare_php_environment() {
    ...
    # For PHP_VERSION=auto/default use distribution packages. This avoids adding
    # PPAs that are not yet available for current Ubuntu releases.
    if [[ "$PHP_VERSION" == "auto" || "$PHP_VERSION" == "default" ]]; then
        info "PHP preparation" "Using distribution default PHP packages."
        if ! apt-get -qq update >/dev/null 2>&1; then
            error "Updating package lists failed."
            return 1
        fi
        return 0
    fi

    # Add PHP repository only for explicitly requested PHP versions.
    if [[ " ${DEBIAN[*]} " =~ " ${OS_CODENAME} " ]]; then
        ...
        info "PHP preparation" "Adding Ondrej PHP PPA for explicitly requested PHP ${PHP_VERSION_EFFECTIVE}."
...
function set_php_version_cli() {
    ...
    if [[ -z "$version" || "$version" == "auto" || "$version" == "default" ]]; then
        if command -v php >/dev/null 2>&1; then
            PHP_VERSION_EFFECTIVE="$(detect_installed_php_version || true)"
            info "PHP CLI" "Using distro default CLI php: $(php -v | head -n1)"
            return 0
        fi
        error "No default PHP CLI found." >&2
        return 1
    fi
    ...
    if [[ ! -x "$php_bin" ]]; then
        if command -v php >/dev/null 2>&1; then
            PHP_VERSION_EFFECTIVE="$(detect_installed_php_version || true)"
            warn "$php_bin not found. Keeping distro default CLI php: $(php -v | head -n1)"
            return 0
        fi
...
function set_php_version_apache() {
    ...
    # Disable any currently enabled mod_php modules before enabling the selected/default one.
    a2dismod -f php* >/dev/null 2>&1 || true

    if [[ -n "$version" && "$version" != "auto" && "$version" != "default" ]]; then
        if a2enmod "php${version}" >/dev/null 2>&1; then
            ...
            if [[ "$HAS_SYSTEMD" == true ]] && systemctl is-active --quiet apache2; then
                systemctl restart apache2 &>/dev/null || confirm "Apache Webserver" "Failed to restart Apache Webserver. Please reboot to apply."
            fi
            return 0
        else
            warn "Could not enable php${version} for Apache. Trying distro default PHP module."
        fi
    fi

    # Generic/default fallback for Ubuntu 26.04+ and future releases.
    if a2enmod php >/dev/null 2>&1; then
        confirm "Apache Webserver" "Apache is now configured to use the distro default PHP module."
        if [[ "$HAS_SYSTEMD" == true ]] && systemctl is-active --quiet apache2; then
            systemctl restart apache2 &>/dev/null || confirm "Apache Webserver" "Failed to restart Apache Webserver. Please reboot to apply."
        fi
        return 0
    ...
    detected_module=$(find /etc/apache2/mods-available -maxdepth 1 -name 'php*.load' -printf '%f\n' 2>/dev/null | sed 's/\.load$//' | sort -V | tail -n 1)
    if [[ -n "$detected_module" ]] && a2enmod "$detected_module" >/dev/null 2>&1; then
        PHP_VERSION_EFFECTIVE="${detected_module#php}"
        confirm "Apache Webserver" "Apache is now configured to use ${detected_module}."
        if [[ "$HAS_SYSTEMD" == true ]] && systemctl is-active --quiet apache2; then
            systemctl restart apache2 &>/dev/null || confirm "Apache Webserver" "Failed to restart Apache Webserver. Please reboot to apply."
        fi
        return 0
    ...
    error "Could not enable any PHP module for Apache" >&2
    return 1
}
...
function toggle_skip_gunicorn() {
    if whiptail --title "Gunicorn Setup" \
        --yesno "Current value: Skip Gunicorn setup = $SKIP_GUNICORN\n\nToggle this option?" 10 50; then
        SKIP_GUNICORN=$([ "$SKIP_GUNICORN" = true ] && echo false || echo true)
        info "Gunicorn Setup" "Skip Gunicorn setup toggled to $SKIP_GUNICORN"
    else
        info "Gunicorn Setup" "No changes made."
    fi
}

function toggle_skip_ngrok() {
    if whiptail --title "ngrok Setup" \
        --yesno "Current value: Skip ngrok setup = $SKIP_NGROK\n\nToggle this option?" 10 50; then
        SKIP_NGROK=$([ "$SKIP_NGROK" = true ] && echo false || echo true)
        info "ngrok Setup" "Skip ngrok setup toggled to $SKIP_NGROK"
    else
        info "ngrok Setup" "No changes made."
    fi
}
...
function general_permissions() {
    ...
    # Change ownership of the installation folder without flattening file modes.
    # Do not chmod the whole project tree: node_modules/.bin and git hooks need
    # executable bits for npm/composer build steps.
    chown -R www-data:www-data "$INSTALLFOLDERPATH"/ >/dev/null 2>&1 || warn "Failed to set ownership for $INSTALLFOLDERPATH"
    chmod +x "$INSTALLFOLDERPATH"/node_modules/.bin/* >/dev/null 2>&1 || true
    sudo -u www-data git config --global --add safe.directory "$INSTALLFOLDERPATH" >/dev/null 2>&1 || true

    # Keep the original Photobooth model: webserver user www-data owns the tree
    # and the installer performs git/npm/composer as www-data. In addition,
    # grant the real login user immediate ACL write access so manual commands
    # such as "npm run build" after installation do not fail with EACCES.
    # This avoids chmod'ing the full tree to broad modes and preserves executable
    # bits in node_modules/.bin.
    if [[ -n "$USERNAME" ]] && id "$USERNAME" >/dev/null 2>&1; then
        if command -v setfacl >/dev/null 2>&1; then
            setfacl -R -m "u:${USERNAME}:rwX" "$INSTALLFOLDERPATH" >/dev/null 2>&1 || warn "Failed to grant $USERNAME ACL access to $INSTALLFOLDERPATH"
            find "$INSTALLFOLDERPATH" -type d -exec setfacl -m "d:u:${USERNAME}:rwX" {} + >/dev/null 2>&1 || warn "Failed to set default ACL for $USERNAME on $INSTALLFOLDERPATH"
        else
            chmod -R g+rwX "$INSTALLFOLDERPATH" >/dev/null 2>&1 || warn "setfacl missing and group-write fallback failed for $INSTALLFOLDERPATH"
        fi
    fi

    # Set permissions on writable runtime folders only
    for runtime_folder in "$INSTALLFOLDERPATH/private" "$INSTALLFOLDERPATH/data" "$INSTALLFOLDERPATH/config" "$INSTALLFOLDERPATH/var"; do
        if [[ -d "$runtime_folder" ]]; then
            chmod -R ug+rwX,o-rwx "$runtime_folder" >/dev/null 2>&1 || warn "Failed to set runtime permissions on $runtime_folder"
            chown -R www-data:www-data "$runtime_folder" >/dev/null 2>&1 || warn "Failed to set runtime ownership on $runtime_folder"
        fi
    done

    # Preserve setgid on private if available
...
function install_usb_sudoers() {
    ...
    # Keep this sudoers fragment intentionally simple and portable.
    # Older versions tried to restrict arguments with complex wildcard command lines;
    # some sudo versions reject that syntax. Polkit remains the preferred path for
    # udisksctl; this sudoers file only covers fallback calls from the Node scripts.
    cat >"$sudoers_file" <<'EOF'
# Photobooth USB Sync fallback permissions for www-data (sync-to-drive.js, move2usb).
Cmnd_Alias PHOTOBOOTH_USB_SYNC = /bin/mount, /usr/bin/mount, /bin/umount, /usr/bin/umount, /bin/mkdir, /usr/bin/mkdir, /usr/bin/udisksctl
...
EOF
    ...
    if visudo -cf "$sudoers_file" >/dev/null 2>&1; then
        ...
        error "Invalid sudoers file created at $sudoers_file. Removing it to keep sudoers clean."
        rm -f "$sudoers_file"
        return 1
    fi
}
...
function check_nodejs() {
    ...
    # Keep the original Photobooth requirement: Node.js 20.x, minimum minor 15.
    # Newer major versions (21+) are not accepted automatically; they trigger
    # the same downgrade path as the original installer, but without interactive apt/gpg prompts.
    if [[ -n "$major" && "$major" =~ ^[0-9]+$ ]]; then
        if [[ "$major" -ge "21" ]]; then
            warn "Node.js downgrade suggested. Photobooth requirement is Node.js v$NODEJS_MAJOR.$NODEJS_MINOR.x."
            return 3
        elif [[ "$major" -eq "$NODEJS_MAJOR" ]]; then
            if [[ -z "$minor" || ! "$minor" =~ ^[0-9]+$ ]]; then
                minor=0
            fi
            if [[ "$minor" -lt "$NODEJS_MINOR" ]]; then
                warn "Node.js update required. Photobooth requirement is Node.js v$NODEJS_MAJOR.$NODEJS_MINOR.x."
                return 1
            fi
    ...
    warn "Node.js version too low. Photobooth requirement is Node.js v$NODEJS_MAJOR.$NODEJS_MINOR.x."
    return 1
}

function update_nodejs() {
    # Keep the original Photobooth target: Node.js 20.x with minimum 20.15.
    # This function may update too-old Node.js or downgrade newer major versions.
    local current_node="0"
    if command -v node >/dev/null 2>&1; then
        current_node=$(node -v 2>/dev/null || echo "0")
    fi
    info "Node.js" "Preparing Node.js v$NODEJS_MAJOR.x installation. Current Node.js: $current_node"

    # Only skip if the installed version satisfies the original requirement:
    # major exactly 20 and minor >= 15. Do NOT accept Node 21/22/24 here.
    local cur_major="" cur_minor=""
    cur_major=$(echo "${current_node#v}" | cut -d. -f1)
    cur_minor=$(echo "${current_node#v}" | cut -d. -f2)
    if [[ "$cur_major" == "$NODEJS_MAJOR" && "$cur_minor" =~ ^[0-9]+$ && "$cur_minor" -ge "$NODEJS_MINOR" ]]; then
        info "Node.js" "Existing Node.js $current_node satisfies the original Photobooth requirement. Skipping Node.js reinstall."
        return 0
    fi

    info "Node.js update" "Removing conflicting Node.js packages if installed."
    DEBIAN_FRONTEND=noninteractive apt-get -qq purge -y         nodejs nodejs-doc npm nsolid node-corepack 'libnode*' >/dev/null 2>&1 || true
    DEBIAN_FRONTEND=noninteractive apt-get -qq autoremove --purge -y >/dev/null 2>&1 || true
    rm -f /usr/bin/node /usr/bin/nodejs /usr/local/bin/node

    info "Node.js" "Installing Node.js v$NODEJS_MAJOR.x from NodeSource."
    ...
    rm -f /etc/apt/keyrings/nodesource.gpg /etc/apt/sources.list.d/nodesource.list /etc/apt/preferences.d/nodesource
    if ! curl -fsSL https://deb.nodesource.com/gpgkey/nodesource-repo.gpg.key | gpg --batch --yes --dearmor -o /etc/apt/keyrings/nodesource.gpg >/dev/null 2>&1; then
        error "Failed to install NodeSource signing key."
        return 1
    fi
    ...
    # Ubuntu 26.04+ can offer a higher distro Node.js (e.g. 22.x). Pin NodeSource
    # above distro packages so apt is allowed to downgrade to the supported 20.x line.
...
EOF
    ...
    if ! DEBIAN_FRONTEND=noninteractive apt-get -qq update >/dev/null 2>&1; then
        error "Failed to update package lists for NodeSource."
        return 1
    fi
    if ! DEBIAN_FRONTEND=noninteractive apt-get -qq install -y --allow-downgrades nodejs >/dev/null 2>&1; then
        error "Failed to install Node.js v$NODEJS_MAJOR.x."
        return 1
    fi

    hash -r
    local installed_node="0" installed_major="" installed_minor=""
    installed_node=$(node -v 2>/dev/null || echo "0")
    installed_major=$(echo "${installed_node#v}" | cut -d. -f1)
    installed_minor=$(echo "${installed_node#v}" | cut -d. -f2)
    if [[ "$installed_major" != "$NODEJS_MAJOR" || ! "$installed_minor" =~ ^[0-9]+$ || "$installed_minor" -lt "$NODEJS_MINOR" ]]; then
        error "Node.js installation finished, but active node is still $installed_node. Required: v$NODEJS_MAJOR.$NODEJS_MINOR.x."
        error "Check 'which node', 'apt-cache policy nodejs', and remove conflicting /usr/local/bin/node if present."
        return 1
    fi

    info "Node.js" "Node.js installed/updated successfully: $installed_node."
    return 0
}

function version_ge() {
    # Compare semantic versions: version_ge "current" "required"
    # Returns 0 if current >= required, else 1.
    local current="$1"
    local required="$2"
    current="${current#v}"
    required="${required#v}"
...
function proof_npm() {
    if ! command -v npm >/dev/null 2>&1; then
        warn "npm command not found."
        return 1
    fi

    local npm_version
    npm_version=$(npm -v 2>/dev/null || echo "0")
    local required_npm="${NPM_MAJOR}.${NPM_MINOR}.${NPM_PATCH}"

    info "npm" "Found npm version: $npm_version ($(command -v npm))."
    info "npm" "Photobooth requires npm >= $required_npm."
    ...
    npm install "npm@$required_npm" -g >/dev/null 2>&1 || warn "[WARN] Failed to upgrade npm globally using npm."

    hash -r
    local npm_version_updated
    npm_version_updated=$(npm -v 2>/dev/null || echo "0")

    if version_ge "$npm_version_updated" "$required_npm"; then
        info "npm" "npm version updated successfully to $npm_version_updated."
        return 0
    fi

    warn "Failed to meet npm version requirements even after attempting update. Continuing may fail during npm install/build."
    confirm "npm Update Warning" "npm version requirements were not met. Installation may fail during npm install/build."
    return 1
}

function check_npm() {
    if command -v npm >/dev/null 2>&1; then
        info "npm" "npm is installed and usable: $(npm -v 2>/dev/null) at $(command -v npm)."
    else
        warn "npm is not installed. Attempting to install npm via apt only because no npm command exists."
        if ! apt-get -qq update >/dev/null 2>&1; then
        ...
        # Prefer npm from NodeSource/node package if possible. Some current Ubuntu systems
        # keep an old apt npm package in state "ic" while /usr/local/bin/npm works.
        # Only install apt npm when no npm command exists at all.
...
function start_git_install() {
    ...
    # Keep these operations under www-data because the repository is installed
    # and updated by the webserver user in this setup. Ensure git and npm
    # helper binaries stay executable; otherwise the frontend build can finish
    # without CSS/JS assets or fail with "Permission denied".
    sudo -u www-data git config --global --add safe.directory "$INSTALLFOLDERPATH" >/dev/null 2>&1 || true
...
function install_or_update_photobooth() {
    ...
    if [ "$SKIP_PHP" = true ]; then
        ...
        if ! set_php_version_cli "$PHP_VERSION_EFFECTIVE"; then
    ...
    if [ "$SKIP_WEBSERVER" = true ]; then
        ...
        if [[ "$setup_apache" = true ]]; then
            ...
            if ! set_php_version_apache "$PHP_VERSION_EFFECTIVE"; then
                confirm "Apache Webserver" "Failed to setup PHP for ${PHP_VERSION_EFFECTIVE:-default}. Ignoring..."
            fi
        fi
    fi
   ...
   if [ "$SKIP_NODE" = true ]; then
        ...
        case $? in
            0)
                ...
                ;;
            1|3)
                # Installed Node.js is too old or too new; install/downgrade to the original supported Node.js 20.x line.
   ...
   if [ "$SKIP_PYTHON" = true ]; then
    ...
    # Zuerst die eigentliche Git-Installation und der Build-Prozess
    start_git_install
    case $? in
        0)
    ...
    if ! create_sumup_token_file; then
        warn "SumUp Setup" "Failed to create empty sumup_token.txt"
    fi

    # SumUp / Zahlungskomponenten erst nach erfolgreicher Photobooth-Installation
    if ! install_sumup_webhook_dependencies; then
        warn "SumUp Setup" "Webhook dependencies (Flask/Requests) installation failed."
    fi

    if [ "$SKIP_GUNICORN" = false ]; then
        if ! install_sumup_gunicorn; then
            warn "SumUp Setup" "Gunicorn installation failed."
        fi
    fi

    if [ "$SKIP_NGROK" = false ]; then
        if ! install_ngrok; then
            warn "SumUp Setup" "ngrok installation failed."
        fi
    fi
    ...
    while true; do
        ...
        case $CHOICE in
            ...
            2)
                ...
                ;;
            3)
                update_php_ini "/etc/php/${PHP_VERSION_EFFECTIVE:-$PHP_VERSION}/apache2/php.ini"
                case $? in
                    0)
...
function show_install_configuration() {
    while true; do
        MENU_OPTIONS=(
            ...
            "7" "Skip Gunicorn setup (current: $SKIP_GUNICORN)" \
            "8" "Skip ngrok setup (current: $SKIP_NGROK)" \
        )
